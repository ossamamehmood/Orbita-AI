...
export default function Dashboard() {
  ...
  return (
    <div className="flex h-screen bg-black text-foreground overflow-hidden relative">
      ...
      <aside 
        className={cn(
          "fixed lg:static inset-y-0 left-0 bg-black/50 backdrop-blur-3xl border-r border-white/5 flex flex-col items-center py-6 sm:py-8 z-50 transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] shadow-2xl shadow-black",
      ...
      >
        <div className={cn("flex items-center mb-10 w-full", isSidebarCollapsed ? "justify-center" : "px-2")}>
                <div className={cn("flex items-center", isSidebarCollapsed ? "gap-0" : "gap-3")}>
                  ...
                  {!isSidebarCollapsed && (
                    <span className="font-display font-bold text-xl tracking-tight text-white whitespace-nowrap">Orbita</span>
                  )}
                </div>
                <Button variant="ghost" size="icon" className="lg:hidden text-white/40 ml-auto" onClick={() => setIsSidebarOpen(false)}>
                  <ChevronLeft className="w-6 h-6" />
                </Button>
              </div>

              <nav className="flex-1 w-full space-y-6 overflow-y-auto scrollbar-none pr-1">
                <div className="space-y-1">
                  {!isSidebarCollapsed && <p className="text-[10px] font-bold uppercase tracking-[0.05em] text-white/20 mb-4 px-4">Menu</p>}
                  <SidebarItem 
                  ...
                  <div className="flex items-center justify-between px-4 mb-4">
                    {!isSidebarCollapsed && (
                      <button 
                        ...
                        className="text-[10px] font-black uppercase tracking-[0.2em] text-white hover:text-gradient hover:scale-105 transition-all outline-none cursor-pointer"
                      >
                        Workspaces
                      </button>
                    )}
                    {!isSidebarCollapsed && (
                      <Button 
                        ...
                        className="w-5 h-5 hover:text-white group transition-colors"
                      >
                  ...
                  {!isSidebarCollapsed && isCreatingWorkspace && (
                    ...
                    >
                      <input 
                        ...
                        className="w-full bg-white/5 border border-primary/20 rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-primary/50 font-bold"
                      />
                    </motion.form>
                  )}
                    ...
                    <div className="space-y-1">
                      ...
                      <div>
                        ...
                        {activeWorkspace === null && !isSidebarCollapsed && (
                          <div className={cn(
                            "ml-[24px] mt-1 space-y-0.5 mb-2 overflow-hidden border-l border-white/5 pl-3",
                            ...
                            <AnimatePresence>
                              {tasks.filter(t => !t.workspaceId && !t.isDeleted && t.status !== 'completed').slice(0, 5).map(task => (
                                <motion.div 
                                  ...
                                  key={task.id} 
                                  className="text-[11px] text-white/40 truncate py-1 relative transition-all flex items-center gap-2 group/task cursor-pointer hover:text-white"
                                >
                                  ...
                                  <div className="w-1 h-1 rounded-full bg-white/20 group-hover/task:futuristic-gradient shrink-0 transition-all shadow-[0_0_8px_rgba(var(--primary-rgb),0)] group-hover/task:shadow-[0_0_8px_rgba(var(--primary-rgb),0.5)]" />
                            ...
                            icon={() => (
                              <div className="relative flex items-center justify-center w-8 h-8 group/folder">
                                <Folder className={cn(
                                  ...
                                  activeWorkspace === ws.id ? "text-white fill-white/20" : "text-white/40 group-hover:text-white group-hover:drop-shadow-[0_0_8px_rgba(2,254,220,0.4)]"
                                )} />
                                <span className={cn(
                                  "absolute top-[11px] font-black text-[10px] uppercase tracking-tighter flex items-center justify-center pointer-events-none",
                                  activeWorkspace === ws.id ? "text-white" : "text-white/60 group-hover:text-white"
                                )}>
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger className={cn(
                    "w-full flex flex-col items-center py-4 px-4 bg-white/[0.03] backdrop-blur-3xl border border-white/5 rounded-3xl mt-auto mb-4 transition-all duration-500 hover:bg-white/[0.06] group/user relative outline-none",
                    ...
                    <div className={cn("flex items-center w-full", isSidebarCollapsed ? "justify-center gap-0" : "gap-3")}>
                      <div className="relative p-[1px] rounded-full group-hover/user:futuristic-gradient transition-all duration-500">
                        <Avatar className={cn(
                          "w-10 h-10 border border-white/10 ring-2 ring-primary/20 group-hover/user:ring-0 transition-all bg-black",
                      ...
                      {!isSidebarCollapsed && (
                        <div className="min-w-0 text-left">
                          <p className="text-sm font-bold text-white truncate group-hover/user:text-gradient transition-all">{userProfile.name}</p>
                          <p className="text-[9px] text-white/30 uppercase tracking-widest font-black">Creator Profile</p>
                        </div>
                      )}
                    </div>
                  </DropdownMenuTrigger>
                ...
                <Button 
                    ...
                    className={cn(
                      "text-white/20 hover:text-white hover:bg-white/5 h-10 rounded-xl transition-all border border-white/5",
                ...
                >
                    ...
                    {!isSidebarCollapsed && <span className="text-[10px] font-black uppercase tracking-widest">Collapse Menu</span>}
                </Button>
      </aside>
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden">
        {/* Header */}
        <header className="h-16 sm:h-20 bg-black/40 border-b border-white/5 px-4 sm:px-6 lg:px-10 flex items-center z-20 backdrop-blur-xl gap-4 sticky top-0">
          {/* Left - Mobile Trigger or Spacer for centering */}
          <div className="flex-none lg:flex-1 flex items-center">
            <Button variant="ghost" size="icon" className="lg:hidden text-white/60 hover:bg-white/5 hover:text-white" onClick={() => setIsSidebarOpen(true)}>
          ...
          <div className="flex-1 lg:flex-[3] flex justify-center min-w-0">
            <div className="w-full max-w-lg relative group">
              <Search className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-white/30 group-focus-within:text-white transition-colors" />
              <input 
                placeholder="Search..." 
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-10 sm:pl-12 pr-4 sm:pr-6 h-9 sm:h-10 bg-white/[0.04] border border-white/5 rounded-full focus:outline-none focus:border-white/20 text-sm transition-all focus:bg-white/[0.07] text-white placeholder:text-white/20 font-medium"
              />
          ...
          <div className="flex-none lg:flex-1 flex items-center justify-end gap-2 sm:gap-4 shrink-0">
            <DropdownMenu onOpenChange={setIsNotificationsOpen}>
              <DropdownMenuTrigger
                className={cn(
                  "relative hover:bg-white/5 rounded-full w-9 sm:w-10 h-9 sm:h-10 transition-all border border-white/5 flex items-center justify-center outline-none",
                  isNotificationsOpen && "bg-white/10 text-white border-white/20 shadow-lg"
                )}
              >
                <Bell className="w-4.5 h-4.5 text-white/60" />
                {notifications.length > 0 && (
                  <span className="absolute -top-1 -right-1 w-4 h-4 sm:w-5 sm:h-5 futuristic-gradient text-[8px] sm:text-[10px] flex items-center justify-center font-black text-white rounded-full border-2 border-black shadow-[0_0_15px_rgba(2,254,220,0.5)] animate-bounce-subtle pointer-events-none">
              ...
              <DropdownMenuContent 
                align="end" 
                sideOffset={16}
                className="w-[280px] sm:w-80 glass-blue-glossy border-white/20 rounded-3xl p-4 sm:p-6 shadow-2xl space-y-6"
              >
                <div className="flex items-center justify-between">
                  <h4 className="font-bold text-white">System Alerts</h4>
                  <Badge className="bg-linear-to-r from-primary/20 to-accent/20 text-white border-0 text-[8px] uppercase tracking-widest px-2">{notifications.length} New</Badge>
                </div>
                <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 scrollbar-none">
                  {notifications.length === 0 ? (
                    <p className="text-[10px] text-white/20 text-center py-4 italic">No active alerts detected</p>
                  ) : (
                    notifications.map((n, idx) => (
                      <div key={idx} className="space-y-1 group cursor-pointer border-b border-white/5 pb-3 last:border-0 hover:border-primary/20 transition-colors">
                        <div className="flex items-center justify-between">
                          <p className="text-xs font-bold text-white group-hover:text-gradient transition-colors">{n.title}</p>
                          <span className="text-[8px] text-white/20 font-bold uppercase">{n.time}</span>
                        </div>
                        <p className="text-[10px] text-white/40 leading-relaxed truncate">{n.desc}</p>
                      </div>
                    ))
                  )}
                </div>
                {notifications.length > 0 && (
                  <Button 
                    ...
                    className="w-full h-10 rounded-xl text-[10px] font-bold uppercase tracking-widest text-white hover:bg-primary/5 transition-colors border border-primary/10 group"
                  >
                    <span className="group-hover:text-gradient">Clear All Logs</span>
                  </Button>
                )}
              </DropdownMenuContent>
            </DropdownMenu>
            <DropdownMenu>
              <DropdownMenuTrigger className="flex items-center gap-2 sm:gap-3 pl-4 sm:pl-6 border-l border-white/5 h-8 cursor-pointer group outline-none shrink-0">
                <Avatar className="w-8 h-8 sm:w-9 sm:h-9 border border-white/10 bg-black/50 ring-1 ring-white/5 group-hover:ring-white/20 transition-all">
                  <AvatarImage src={userProfile.photo || undefined} />
                  <AvatarFallback className="bg-white/5 text-white/60 font-bold text-[10px] uppercase flex items-center justify-center">
                    {userProfile.name.substring(0, 2)}
                  </AvatarFallback>
                </Avatar>
                <div className="text-left hidden sm:block">
                  <p className="text-xs font-semibold text-white/90 group-hover:text-white transition-all whitespace-nowrap">{userProfile.name}</p>
                </div>
              </DropdownMenuTrigger>
              <DropdownMenuContent className="glass-blue-glossy border-white/20 min-w-[240px] p-2 rounded-[1.5rem] shadow-2xl mt-4" align="end">
                <div className="px-4 py-3 border-b border-white/10 mb-2">
                   <p className="text-[10px] font-black uppercase tracking-widest text-gradient">Biometric Identity</p>
                   <p className="text-sm font-bold text-white mt-1 truncate">{userProfile.name}</p>
                </div>
                <DropdownMenuItem onClick={() => setActiveTab('dashboard')} className="rounded-xl py-3 px-4 hover:bg-white/5 cursor-pointer font-bold text-xs uppercase tracking-widest gap-3 group/item">
                   <LayoutDashboard className="w-4 h-4 text-blue-400 group-hover/item:text-primary transition-colors" />
                   Orbita Dashboard
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setActiveTab('workspaces')} className="rounded-xl py-3 px-4 hover:bg-white/5 cursor-pointer font-bold text-xs uppercase tracking-widest gap-3 group/item">
                   <Layers className="w-4 h-4 text-blue-500 group-hover/item:text-primary transition-colors" />
                   Workspace Control
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setActiveTab('settings')} className="rounded-xl py-3 px-4 hover:bg-white/5 cursor-pointer font-bold text-xs uppercase tracking-widest gap-3 group/item">
                   <Settings2 className="w-4 h-4 text-primary group-hover/item:text-blue-400 transition-colors" />
                   User Settings
                </DropdownMenuItem>
                <div className="h-[1px] bg-white/5 my-2 mx-2" />
                <DropdownMenuItem onClick={() => resetSystem()} className="rounded-xl py-3 px-4 hover:bg-red-500/10 text-red-400 cursor-pointer font-bold text-xs uppercase tracking-widest gap-3">
              ...
              {activeTab === "settings" ? (
                ...
                <div className="max-w-6xl mx-auto space-y-12 sm:space-y-16">
                  ...
                  <section className="relative group">
                    ...
                    <div className="relative glass-glossy rounded-[2.5rem] p-8 sm:p-12 overflow-hidden border-white/5">
                      <div className="absolute top-4 right-4 sm:top-6 sm:right-6 opacity-10 group-hover:opacity-20 transition-opacity pointer-events-none">
                         <OrbitaLogo className="w-48 h-48 sm:w-64 sm:h-64 rotate-12" />
                      </div>
                      <div className="relative z-10 space-y-6">
                        <div className="space-y-2">
                          <h2 className="text-4xl sm:text-5xl md:text-6xl font-display font-medium tracking-tight text-white leading-[1.1]">
                            Good morning, <br />
                            <span className="text-gradient font-bold drop-shadow-[0_0_30px_rgba(2,254,220,0.3)]">{userProfile.name}</span>
                          </h2>
                        </div>
                        <p className="text-white/40 text-lg sm:text-xl font-medium max-w-xl leading-relaxed">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8">
                    {stats.map((stat, i) => (
                      <motion.div 
                        ...
                        key={i} 
                        className="p-6 sm:p-8 rounded-[2rem] glass border-white/5 flex items-center justify-between group hover:border-white/20 hover:bg-white/[0.04] transition-all duration-500 relative overflow-hidden shadow-2xl"
                      >
                        ...
                        <div className="space-y-1.5 relative z-10">
                          <p className="text-[11px] font-black uppercase tracking-[0.2em] text-white/30 group-hover:text-white/50 transition-colors">{stat.label}</p>
                          <p className={`text-3xl sm:text-4xl font-display font-black tabular-nums tracking-tight ${stat.label === "Active Tasks" ? "text-gradient" : "text-white"}`}>{stat.value}</p>
                        </div>
                        <div className={cn(
                          "w-12 h-12 sm:w-16 sm:h-16 rounded-2xl bg-white/5 flex items-center justify-center border border-white/10 group-hover:bg-white/10 group-hover:scale-110 transition-all duration-700 relative shadow-inner overflow-hidden",
                          ...
                          <stat.icon className={cn(
                            ...
                            stat.label === "Active Tasks" ? "stroke-[url(#futuristic-gradient)]" : stat.color + " opacity-60 group-hover:opacity-100"
                          )} />
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-12 pt-4">
                      <div className="lg:col-span-3 space-y-10">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                          <div className="space-y-2">
                            <h3 className="text-3xl font-display font-black text-white tracking-tight">{activeTab === 'completed' ? 'Synced Tasks' : 'Active Tasks'}</h3>
                            <p className="text-base text-white/40 font-medium">{activeTab === 'completed' ? 'Successfully archived workflows' : 'Ongoing tasks within the system'}</p>
                          </div>
                          <div className="flex gap-4">
                            {activeTab === 'completed' && filteredTasks.length > 0 && (
                              <Button 
                                ...
                                className="text-[11px] font-black uppercase tracking-widest h-10 px-6 rounded-xl border border-red-500/20 text-red-400 hover:bg-red-500/10 shadow-lg shadow-red-500/5 transition-all"
                              >
                                Decommission Synced Tasks
                              </Button>
                            )}
                            <DropdownMenu>
                              <DropdownMenuTrigger className={cn(buttonVariants({ variant: "ghost", size: "sm" }), "text-[11px] font-black uppercase tracking-widest h-10 px-6 rounded-xl border border-white/10 hover:bg-white/10 shadow-xl")}>Sort By: Priority</DropdownMenuTrigger>
                              <DropdownMenuContent className="glass-blue-glossy border-white/20 text-white p-2 rounded-2xl">
                                <DropdownMenuItem className="hover:bg-white/5 cursor-pointer rounded-xl font-bold py-2.5">Priority</DropdownMenuItem>
                                <DropdownMenuItem className="hover:bg-white/5 cursor-pointer rounded-xl font-bold py-2.5">Date Created</DropdownMenuItem>
                                <DropdownMenuItem className="hover:bg-white/5 cursor-pointer rounded-xl font-bold py-2.5">Status</DropdownMenuItem>
                              </DropdownMenuContent>
                            </DropdownMenu>
                          </div>
                        </div>
                      ...
                      <div className="space-y-12">
                        <section>
                          <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-4">
                            <h3 className="text-lg font-display font-bold text-white tracking-tight uppercase">System Registry</h3>
                          </div>
                          <div className="space-y-6">
                            {tasks.filter(t => !t.isDeleted).slice(0, 8).map(task => (
                              <div key={task.id} className="flex items-center gap-5 group cursor-pointer">
                                <div className={`w-2 h-10 rounded-full ${task.status === 'completed' ? 'futuristic-gradient shadow-[0_0_10px_rgba(2,254,220,0.5)]' : 'bg-white/5 group-hover:bg-linear-to-b group-hover:from-primary/40 group-hover:to-accent/40'} transition-all duration-300`} />
                                <div className="flex-1 min-w-0">
                                  <p className="text-base font-bold truncate group-hover:text-gradient transition-colors text-white/80 uppercase tracking-tight">{task.title}</p>
                                  <p className="text-[10px] text-white/20 uppercase tracking-[0.2em] font-black mt-1">Status: {task.status.toUpperCase()}</p>
                                </div>
          ...
          isAddingTask ? "max-w-3xl" : "max-w-md"
           ...
           {isAddingTask ? (
             <motion.form 
               ...
               className="glass-blue-glossy rounded-[2rem] p-5 sm:p-6 flex flex-col gap-5 border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative overflow-hidden"
             >
                <div className="absolute top-0 right-0 p-4 sm:p-5 z-10">
                   <Button 
                    ...
                    className="text-white/20 hover:text-white h-7 w-7 rounded-full"
                   >
                    <X className="w-4 h-4" />
                   </Button>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    <div className="w-1.5 h-6 rounded-full futuristic-gradient shadow-[0_0_10px_rgba(2,254,220,0.5)]" />
                    <h3 className="text-sm font-black uppercase tracking-[0.3em] text-white/40">New Workflow Cycle</h3>
                  </div>
                  
                  <div className="bg-black/60 backdrop-blur-2xl border border-white/10 rounded-2xl sm:rounded-[2rem] p-1 sm:p-2 shadow-inner group/neural">
                    <textarea 
                      autoFocus
                      placeholder="Input neural data..."
                      value={quickTaskTitle}
                      onChange={(e) => setQuickTaskTitle(e.target.value)}
                      className="w-full bg-transparent border-none focus:outline-none text-white font-medium p-3 sm:p-4 text-base sm:text-lg min-h-[100px] sm:min-h-[120px] max-h-[300px] scrollbar-none placeholder:text-white/10 resize-none group-focus-within/neural:placeholder:text-white/20 transition-all font-sans"
                    />
                  </div>
                </div>

                <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
                  <div className="flex items-center gap-3 bg-white/5 px-4 sm:px-6 rounded-[1.2rem] sm:rounded-[1.5rem] border border-white/10 focus-within:border-white/40 focus-within:bg-white/10 transition-all shadow-xl h-12 sm:h-14 w-full sm:w-auto group/qdate">
                    <Clock className="w-4 h-4 sm:w-5 sm:h-5 text-primary group-hover/qdate:scale-110 transition-transform" />
                    <input 
                      type="date"
                      value={quickTaskDueDate}
                      onChange={(e) => setQuickTaskDueDate(e.target.value)}
                      className="bg-transparent text-[10px] sm:text-xs uppercase font-black text-white/60 group-focus-within/qdate:text-white border-none outline-none py-2 cursor-pointer dark-calendar-picker flex-1 min-w-0"
                    />
                    <span className="text-[10px] font-black uppercase tracking-widest text-white/20 hidden lg:block">Deadline</span>
                  </div>
                ...
                <div className="flex flex-wrap items-center gap-2 px-1 pt-2">
                  <span className="text-[10px] font-black uppercase tracking-widest text-white/20 mr-2">Target Sector:</span>
                  {workspaces.map(ws => (
                    <button
                      key={ws.id}
                      type="button"
             ...
             <Button 
               ...
               className="w-full h-14 sm:h-16 rounded-[2rem] glass-blue-glossy border-white/20 text-white shadow-2xl shadow-primary/10 group transition-all hover:scale-[1.02] hover:border-white/40 flex items-center justify-between px-8 relative overflow-hidden"
             >
                ...
                <div className="flex items-center gap-4 relative z-10">
                  <div className="p-2 rounded-xl bg-white/5 border border-white/10 group-hover:border-white/50 transition-all shadow-inner">
                    <Plus className="w-5 h-5 text-primary group-hover:rotate-90 transition-transform shrink-0" />
                  </div>
                  <span className="font-bold text-base sm:text-lg tracking-tight text-white/90 group-hover:text-white transition-all">Initiate New Task Cycle</span>
                </div>
                <div className="flex items-center gap-2 opacity-50 group-hover:opacity-100 transition-all hidden sm:flex relative z-10">
                   ...
                   <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 group-hover:text-white">Active Uplink</span>
                </div>
             </Button>
           )}
        </div>
          ...
          {isConfirmOpen && confirmOptions && (
            <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6">
              ...
              <motion.div 
                ...
                className="relative w-full max-w-lg glass-blue-glossy rounded-[3rem] p-10 border-white/20 shadow-[0_0_100px_rgba(90,92,255,0.25)] space-y-8 overflow-hidden"
              >
                ...
                <div className="space-y-4">
                  ...
                  <h3 className="text-3xl font-bold text-white tracking-tight">{confirmOptions.title}</h3>
                  <p className="text-white/50 text-lg leading-relaxed">{confirmOptions.description}</p>
                </div>

                {confirmOptions.impact && (
                  <div className="p-6 rounded-2xl bg-white/[0.03] border border-white/5">
                    <div className="text-[10px] font-bold uppercase tracking-widest text-white/30 mb-2">Impact Assessment</div>
                    <p className="text-sm font-medium text-red-400/80">{confirmOptions.impact}</p>
                  </div>
                )}
                ...
                <div className="flex flex-col sm:flex-row gap-4 pt-4">
                  <Button 
                    onClick={closeConfirm}
                    variant="ghost"
                    className="flex-1 h-14 rounded-2xl font-bold uppercase tracking-widest text-white/40 hover:bg-white/5"
                  >
                    Abort Action
                  </Button>
                  <Button 
                    ...
                    className="flex-1 h-14 rounded-2xl futuristic-gradient text-white border-0 font-bold uppercase tracking-widest shadow-xl shadow-primary/20"
                  >
    ...
    >
      <button 
        ...
        className={cn(
          ...
          active 
            ? "glass futuristic-gradient text-white shadow-lg shadow-primary/20" 
            : "text-white/50 hover:text-white hover:bg-linear-to-br hover:from-white/[0.08] hover:to-primary/[0.05]",
      ...
      >
        ...
        {!collapsed && badgeCount !== undefined && (
          <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-white/5 text-white/30 group-hover:bg-white/10 group-hover:text-white transition-colors">
            {badgeCount}
          </span>
        )}
        {isOver && (
