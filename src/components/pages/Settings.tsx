    ...
    <div className="max-w-4xl mx-auto space-y-12">
      <header className="space-y-6">
        <div className="flex items-center gap-5">
          ...
          <div className="space-y-1">
            <h1 className="text-5xl font-display font-bold tracking-tighter text-white">System Config</h1>
            <p className="text-white/40 text-sm font-bold uppercase tracking-[0.3em]">Core Operational Parameters v4.2</p>
          </div>
        </div>
      </header>

      <section className="space-y-6">
        <h3 className="text-[10px] font-bold uppercase tracking-[0.4em] text-white/20 px-6">Operator Signature</h3>
        <div className="glass-glossy rounded-[3rem] p-10 border-white/10 flex flex-col md:flex-row items-center gap-12 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-primary/5 blur-[100px] pointer-events-none" />
          
          <div className="relative shrink-0 group">
            <div className="w-36 h-36 rounded-[2.5rem] p-1.5 glass-glossy border-white/20 relative cursor-pointer overflow-hidden">
              <label className="cursor-pointer">
                ...
                <div className="absolute inset-0 bg-black/60 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center gap-2">
                  <Camera className="w-8 h-8 text-primary shadow-[0_0_15px_rgba(236,72,153,0.5)]" />
            ...
            {/* Status dot */}
            <div className="absolute -bottom-1 -right-1 w-6 h-6 rounded-full bg-primary border-4 border-[#020205] shadow-[0_0_12px_rgba(2,254,220,0.5)] z-20" />
          </div>
          
          <div className="flex-1 w-full space-y-6 relative z-10">
            <div className="space-y-2">
              <label className="text-[10px] font-bold uppercase tracking-[0.3em] text-white/30 px-1">Identity Tag</label>
              <Input 
                value={userProfile.name}
                onChange={(e) => updateProfile({ name: e.target.value })}
                className="bg-white/[0.03] border-white/10 rounded-2xl h-14 focus:border-primary/40 focus:ring-0 text-white font-bold text-xl px-6"
              />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-bold uppercase tracking-[0.3em] text-white/30 px-1">Functional Directive</label>
              <Input 
                value={userProfile.bio}
                onChange={(e) => updateProfile({ bio: e.target.value })}
                className="bg-white/[0.03] border-white/10 rounded-2xl h-14 focus:border-primary/40 focus:ring-0 text-white font-medium px-6"
              />
            </div>
          </div>
        </div>
      ...
      <div className="grid gap-10">
        {sections.map((section, idx) => (
          <section key={idx} className="space-y-6">
            <h3 className="text-[10px] font-bold uppercase tracking-[0.3em] text-white/20 px-4">{section.title}</h3>
            <div className="space-y-3">
              {section.items.map((item, i) => (
                <div 
                  key={i} 
                  className="group flex items-center justify-between p-7 rounded-[2.5rem] glass-glossy border-white/10 transition-all cursor-pointer bg-white/[0.01] hover:bg-white/[0.05] hover:translate-x-2"
                >
                  <div className="flex items-center gap-6">
                    <div className="w-14 h-14 rounded-2xl bg-white/5 flex items-center justify-center group-hover:scale-110 group-hover:bg-white/10 transition-all duration-500 shadow-xl">
                      <item.icon className="w-7 h-7 text-white/40 group-hover:text-primary transition-colors" />
                    </div>
                    <div>
                      <p className="text-base font-bold text-white tracking-tight">{item.label}</p>
                      <p className="text-[10px] text-white/30 font-bold uppercase tracking-widest mt-1">{item.value}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-5">
      ...
      <div className="pt-8 border-t border-white/10 flex flex-col sm:flex-row justify-between items-center gap-6">
        ...
        <div className="flex gap-4">
          <Button variant="ghost" onClick={() => updateProfile({ isFirstTime: true })} className="rounded-2xl h-12 px-8 font-bold text-white/40 hover:text-white transition-colors">Start Onboarding</Button>
